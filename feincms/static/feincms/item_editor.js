...
;(($) => {
  ...
  function create_new_item_from_form(form, modname, modvar) {
    const fieldset = $("<fieldset>").addClass(
      `module aligned order-item item-wrapper-${modvar}`,
    )
    const original_id_id = `#id_${form.attr("id")}-id`
  ...
  function update_item_controls(item, target_region_id) {
    const item_controls = item.find(".item-controls")
    item_controls.empty()

    // Insert control unit
    const insert_control = $("<div>").addClass("item-control-unit")
    const select_content = SELECTS[REGION_MAP[target_region_id]].clone()

    select_content.change(() => {
      const modvar = select_content.val()
      const modname = select_content.find("option:selected").html()
      const new_fieldset = create_new_fieldset_from_module(modvar, modname)
  ...
  function create_new_fieldset_from_module(modvar, modname) {
    const new_form = create_new_spare_form(modvar)
  ...
  function set_item_field_value(item, field, value) {
    ...
    if (field === "delete-field") item.find(`.${field}`).attr("checked", value)
      ...
      const old_region_id = REGION_MAP.indexOf(item.find(`.${field}`).val())
      item.find(`.${field}`).val(REGION_MAP[value])

      // show/hide the empty machine message in the source and
      // target region.
      const old_region_item = $(`#${REGION_MAP[old_region_id]}_body`)
      ...
      const new_region_item = $(`#${REGION_MAP[value]}_body`)
  ...
  function sort_by_ordering(e1, e2) {
    const v1 = parseInt($(".order-field", e1).val(), 10) || 0
    const v2 = parseInt($(".order-field", e2).val(), 10) || 0
    return v1 > v2 ? 1 : -1
  }

  function give_ordering_to_content_types() {
    for (let i = 0; i < REGION_MAP.length; i++) {
      const container = $(`#${REGION_MAP[i]}_body div.order-machine`)
  ...
  function hide_form_rows_with_hidden_widgets() {
    ...
    $("div.feincms_inline div.form-row").each(function () {
      const child_count = $(this).find("*").length
      const invisible_types = "div, label, input[type=hidden], p.help"
      const invisible_count = $(this).find(invisible_types).length
  ...
  function init_content_type_buttons() {
    $("#feincmsmain > .panel").each(function () {
      const $select = $("select[name=order-machine-add-select]", this),
        to_remove = []

      $select.change(() => {
        const modvar = $select.val()
        // bail out early if no content type selected
        if (!modvar) return

        const modname = $select.find("option:selected").html()
        const new_fieldset = create_new_fieldset_from_module(modvar, modname)
  ...
  function create_tabbed(_tab_selector, _main_selector, _switch_cb) {
    const tab_selector = _tab_selector,
  ...
  $(document).ready(($) => {
    ...
    const options_fieldsets = $("fieldset.collapse")
    ...
    options_fieldsets.each((idx, elem) => {
      ...
      const id_base = `extension_option_${idx}`

      const paren = option_title.indexOf(" (")
      ...
      let divs
      if (details.length === 1) {
        /* dj 5.1: ModelAdmin.fieldsets and InlineModelAdmin.fieldsets now includes <details> and <summary> elements */
    ...
    $(document.body).on("click", "h2 img.item-delete", function () {
      const item = $(this).parents(".order-item")
      if (confirm(feincms_gettext("DELETE_MESSAGE"))) {
        const in_database = item.find(".delete-field").length
        if (in_database === 0) {
          // remove on client-side only
          const id = item.find(".item-content > div").attr("id")

          // poorify all contents
          const items = item.parents(".order-machine").find(".order-item")
          items.each(function () {
            poorify_rich($(this))
          })
        ...
        item.fadeOut(200, () => {
          const region_item = $(`#${REGION_MAP[window.ACTIVE_REGION]}_body`)
          if (
    ...
    function on_template_key_changed() {
      const new_template = this.value
      const form_element = $(this).parents("form")

      if (current_template === new_template)
        // Selected template did not change
        return false

      const current_regions = template_regions[current_template]
      const new_regions = template_regions[new_template]
    ...
    const template_key_radio = $("input[type=radio][name=template_key]")
    template_key_radio.click(on_template_key_changed)
    const template_key_select = $("select[name=template_key]")
    ...
    $(".order-machine").sortable({
      handle: ".handle",
      helper(_event, ui) {
        const h2 = $("<h2>").html($(ui).find("span.modname").html())
    ...
    const errors = $("#feincmsmain div.errors")

    if (errors.length) {
      const id = errors
        .parents("fieldset[id$=_body], div[id$=_body]")
        .attr("id")
      $(`#${id.replace("_body", "_tab")}`).trigger("click")
    } else {
      if (window.location.hash) {
        const tab = $(`#${window.location.hash.substr(5)}_tab`)
  ...
  window.ItemEditor = {
    add_content(type, region) {
      const new_fieldset = create_new_fieldset_from_module(
        type,
        CONTENT_NAMES[type],
      )
