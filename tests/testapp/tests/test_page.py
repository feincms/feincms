...
class PagesTestCase(TestCase):
    ...
    def test_17_page_template_tags(self):
        ...
        self.assertEqual(
            t.render(context), "en:/test-page/,de:/test-page/test-child-page/"
        )
        ...
        self.assertEqual(
            t.render(context), "en:/test-page/test-child-page/page3/,de:None"
        )
...
"""
        ...
        data = (
            ...
            .render(template.Context({"feincms_page": p}))
            .strip(),
        )
        self.assertEqual(
            data, ("1,2,3,4,6,7,8,10,11,12,13,14,15,16,18",), "Original navigation"
        )
...
"""
    ...
    def test_19_page_manager(self):
        ...
        self.assertRaises(
            Http404,
            lambda: Page.objects.best_match_for_path("/blabla/blabla/", raise404=True),
        )
        self.assertRaises(
            Http404, lambda: Page.objects.page_for_path("/asdf/", raise404=True)
        )
        self.assertRaises(
            Page.DoesNotExist,
        ...
        self.assertRaises(
            Page.DoesNotExist, lambda: Page.objects.page_for_path("/asdf/")
        )
        ...
        self.assertRaises(
            Http404, lambda: Page.objects.for_request(request, raise404=True)
        )
    ...
    def test_25_applicationcontent(self):
        ...
        page.applicationcontent_set.create(
            region="main", ordering=0, urlconf_path="testapp.applicationcontent_urls"
        )

        self.assertContains(self.client.get(page.get_absolute_url()), "module_root")
        self.assertContains(
            self.client.get(page.get_absolute_url() + "args_test/abc/def/"), "abc-def"
        )
        self.assertContains(
            self.client.get(page.get_absolute_url() + "kwargs_test/abc/def/"), "def-abc"
        )
        ...
        self.assertContains(
            response, "args:/test-page/test-child-page/args_test/xy/zzy/"
        )
        ...
        # This should not raise
        self.assertEqual(
            self.client.get(page.get_absolute_url() + "notexists/").status_code, 404
        )
        ...
        page1.applicationcontent_set.create(
            region="main", ordering=0, urlconf_path="whatever"
        )
    ...
    def test_26_page_form_initial(self):
        ...
        self.assertEqual(
            self.client.get("/admin/page/page/add/?parent=1").status_code, 200
        )
        self.assertEqual(
            self.client.get("/admin/page/page/add/?parent=2").status_code, 200
        )
    ...
    def test_27_cached_url_clash(self):
        ...
        self.login()
        self.assertContains(
            self.create_page_through_admincontent(page2, active=True, override_url="/"),
            "already taken by",
        )
    ...
    def test_29_medialibrary_admin(self):
        ...
        self.assertContains(
            self.client.get("/admin/medialibrary/mediafile/"), "somefile.jpg"
        )
        ...
        with open("test.zip", "rb") as handle:
            response = self.client.post(
                "/admin/medialibrary/mediafile/mediafile-bulk-upload/", {"data": handle}
            )
        ...
        with open(path, "rb") as handle:
            response = self.client.post(
                "/admin/medialibrary/mediafile/add/",
                {
                    "file": handle,
        ...
        self.assertContains(
            self.client.get("/admin/medialibrary/mediafile/"), "100x100"
        )
...
{% for p in nav %}{{ p.get_absolute_url }},{% endfor %}
    ...
    def test_40_page_is_active(self):
        ...
        self.assertTrue(
            feincms_page_tags.page_is_active({"feincms_page": page1}, page1)
        )
        self.assertTrue(
            feincms_page_tags.page_is_active({"feincms_page": page2}, page1)
        )
        self.assertFalse(
            feincms_page_tags.page_is_active({"feincms_page": page1}, page2)
        )

        p = PagePretender(title="bla", slug="bla", url="/test-page/whatsup/")

        self.assertTrue(
            feincms_page_tags.page_is_active({}, p, path="/test-page/whatsup/test/")
        )
        self.assertFalse(feincms_page_tags.page_is_active({}, p, path="/test-page/"))

        self.assertTrue(
            feincms_page_tags.page_is_active(
    ...
    def test_41_templatecontent(self):
        ...
        page.templatecontent_set.create(
            region="main", ordering=10, template="templatecontent_1.html"
        )

        # The empty form contains the template option.
        self.login()
        self.assertContains(
            self.client.get(reverse("admin:page_page_change", args=(page.id,))),
